...
5. Do NOT modify `structural-model.json` after it is locked.
...
- **Do not write to structural-model.json.** The orchestrator locks it at the end of Stage 1. Output brace geometry to `context/staging/bracing-model.json` instead.
...
1. **Standard tall-post hex:** Given structural-model.json with 8ft posts: outputs brace_coordinates in bracing-model.json, brace hardware appended to joinery-model.json. ✓
